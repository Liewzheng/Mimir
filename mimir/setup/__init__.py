...
twice does not create duplicate hooks.
"""
...
class KimiCodeSetup:
    ...
    def is_installed(self) -> bool:
        if not self.config_path.exists():
            return False
        return "mimir.hooks.mimir_turn" in self.config_path.read_text()
    ...
    def install(self) -> Path:
        ...
        if self.config_path.exists():
            ...
            if "mimir.hooks.mimir_turn" in existing:
...
class ClaudeCodeSetup:
    ...
    def install(self) -> Path:
        ...
        for event in ("UserPromptSubmit", "Stop", "SessionStart"):
            ...
            if not any("mimir_turn" in json.dumps(h) for h in hooks):
                hooks.append(mimir_hook)
