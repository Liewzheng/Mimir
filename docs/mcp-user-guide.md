...
> 每轮对话结束时，Mimir 已通过 hook 自动保存了本轮内容。你可以直接回复，无需额外调用 store。系统提示（`<system-reminder>`）和 hook 输出会被过滤，不会存入记忆。
...
## 一键配置：`mimir setup <agent>`

不想手动编辑 JSON/TOML，可以直接用 CLI 安装 hook：
...
使用 `--base-dir` 可将配置写到非默认目录（例如自定义 dotfiles 或 CI）：

```bash
mimir setup kimi-code --base-dir ~/my-configs/kimi-code
```

该命令幂等，重复执行不会重复添加 hook。

---

## 项目上下文自动发现

会话启动时，Mimir 会自动扫描工作空间根目录下的 `AGENTS.md`、`CLAUDE.md`、`.cursorrules`、`.claude.md`，将其作为高重要性记忆注入。这样 agent 无需手动复制粘贴项目规范。

- 仅读取工作空间根目录下的常规文件，忽略符号链接和超出大小限制的文件。
- 可通过 `MimirConfig(project_context_enabled=False)` 关闭。
- 可通过 `MimirConfig(project_context_importance=...)` 调整重要性。
...
| 方式 | 说明 | 适用工具 |
|------|------|----------|
| **Hook（推荐）** | 在 agent 的 `UserPromptSubmit` / `Stop` / `SessionStart` 事件中调用 `mimir.hooks.mimir_turn`，自动 recall、学习，并对最后轮次进行 small-talk 过滤、敏感信息脱敏和重复记忆检查。 | Kimi Code、Claude Code、Codex |
| **Skill / instructions** | 通过系统提示让模型自主调用 store/recall。当 agent 不支持 hook 时使用。 | OpenCode、Kimi Code、Aider、Continue.dev |

无论通过哪种方式写入，Mimir 都会在存储前：

1. **脱敏**：自动替换 API key、token、密码等敏感信息为 `[REDACTED]`。
2. **过滤**：hook 路径会过滤 small-talk；MCP `store()` 只过滤空值和乱码，保留用户意图。
3. **去重**：在 hook 和 MCP 路径中，如果新文本与已有记忆几乎重复（默认 cosine ≥ 0.95），则跳过该记忆，避免重复积累。
...
| 工具 | 参数 | 说明 |
|------|------|------|
| `store` | `text: string`, `importance: float = 1.0` | 存储并学习一条文本。显式调用时不过滤 small-talk（保留用户意图），但会拒绝空值/乱码，并对敏感信息脱敏。如果与已有记忆几乎重复（默认 cosine ≥ 0.95），会返回 `reason: "duplicate"`。 |
